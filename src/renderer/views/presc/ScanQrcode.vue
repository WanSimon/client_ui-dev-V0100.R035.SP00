<template>
  <div class="container">
    ...
    <div class="content">
      <div class="left-item">
        <div class="title">扫二维码识别</div>
        <div class="image"><img :src="image" class="img" /></div>
        <div class="desc">请打开微信公众号，将<span style="color: #4A90E2">就诊卡</span>二维码对准扫描区域, 或手动输入电子就诊卡卡号</div>
      </div>
...
export default {
  ...
  data(){
    return {
      ...
      audio: '/请打开导诊单右上角二维码将导诊单二维码对准扫描区域.mp3',
      mod: 'electronic',
      ...
      try{
        ...
        if(equipmentInfo && equipmentInfo.id) {
          ...
          if(info.equipment_info){
            ...
            if(info.equipment_info.id){
              equipment_id = info.equipment_info.id;
